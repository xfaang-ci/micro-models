...
**Sweep skali (🕵️ PO AUDYCIE 2026-06-21; probe realny + właściwy null):** CKA trenowane **0,915 → 0,936 → 0,947 → 0,945** (wysoko, wcześnie, lekki wzrost → plateau), **≫ null losowy-vs-losowy 0,44**. ⚠️ **mutual-kNN WYCOFANY:** jego null losowy-vs-losowy = **0,68 ≈ trenowane 0,66** → metryka skażona strukturą wejścia, NIE mierzy wyuczonej zgodności; wcześniejsze „kNN rośnie = PRH" było artefaktem probe ×3. **Czysty sygnał daje wyłącznie CKA.** Reframe: same-data+seed = **stabilność względem seeda**, nie platońska konwergencja. Pełny audyt: [[Audyt-Analizy-Pomiaru]]. Figura: `paper/paper.tex` (`fig:cka`).
